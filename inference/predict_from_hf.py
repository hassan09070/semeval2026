"""
Standalone prediction script that loads model from Hugging Face and generates predictions.
"""
...
BASE_MODEL = "microsoft/mdeberta-v3-base"  # Use original for tokenizer
SUBTASK = "subtask_1"
TASK = "task1"
...
HF_TOKEN = os.environ.get("HF_TOKEN")  # Set to your token if model is private
...
def main():
    ...
    # Load tokenizer from base model (original files are correct)
    try:
    ...
    # Load model architecture from base model
    try:
        model = TransformerVARegressor(model_name=BASE_MODEL).to(device)
        print(f"✅ Model architecture loaded from {BASE_MODEL}")
    except Exception as e:
        print(f"❌ Error creating model: {e}")
        return
    
    # Load custom weights from HuggingFace repo
    try:
        from huggingface_hub import hf_hub_download
        model_path = hf_hub_download(repo_id=MODEL_REPO, filename="full_model.bin", token=HF_TOKEN)
        state_dict = torch.load(model_path, map_location=device)
        model.load_state_dict(state_dict, strict=False)
        model.eval()
        print(f"✅ Custom model weights loaded from {MODEL_REPO}")
    except Exception as e:
        print(f"⚠️  Could not load custom weights: {e}")
        print(f"   Will use base model for predictions")
        model.eval()
    ...
    for lang in LANGS:
        for domain in DOMAINS:
            try:
                ...
                # Save to JSONL
                output_file = os.path.join(SUBTASK, f"pred_{lang}_{domain}.jsonl")
